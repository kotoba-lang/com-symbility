(ns symbility.main
  "Kotodama WASM entrypoint for the Symbility clean-room actor (L4) — Clojure port.
  ...
  facts (`symbility.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "symbility")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "symbilit_acc"
    :fields [:holderId :type :balance :currency :status]  :required [:type :balance]
    :coerce {:balance :float}            :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "symbilit_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"symbility.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "symbility-compat" :tier tier :entities entities} 200])
...
(defn start! [] :symbility-compat/ready)
